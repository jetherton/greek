...
class Reports_Controller extends Main_Controller {
	...
	public function submit($id = FALSE, $saved = FALSE)
	{
		...
		if ($_POST)
		{
			...
			{
				//these are the strings to be encrypted
				$strings = array(
					...
					if(!is_array($value)){
						if(array_key_exists($key, $strings)){
							if(strlen($value) >= $maxlength){
								$s = array();
								$v = $value;
								while(strlen($v) >= $maxlength){
									$first = substr($v, 0, $maxlength);
									openssl_public_encrypt($first, $sub, $pub_key);
									$s[] = base64_encode($sub);
									$v = substr($v, $maxlength);
								}
								$str = '';
								...
								if($value != ''){
									...
									$fields_string .= $key.'='.urlencode(base64_encode($s)).'&';
								}
								else{
									$fields_string .= $key.'=&';
								}
	...
	public function rating($id = false)
	{
		/******************************************************************************
		 * GREEK CHANGE
		 * we don't want this on our site, so if someone gets here we just redirect them
		 *****************************************************************************/
		url::redirect('/');
