...
/**
 * POP3 library. Wrapper to read email using POP3.
 * @package    Pop3
...
class Pop3_Core {
									...
									if($this->connection!=0)
									{
										...
										|| $this->quit_handshake)
													...
													if($this->PutLine("QUIT")==0)
															...
															Pop3_Core::SetConnection(-1, $this->connection_name, $this);
															return("");
															}
	...
	if($this->PutLine("PASS $password")==0)
				...
				if($this->state!="TRANSACTION")
					...
					if($this->state!="TRANSACTION")
						...
						if(GetType($response)!="string")
								...
								Function RetrieveMessage($message,&$headers,&$body,$lines, &$raw)
								{
									$raw= '';
								if($this->state!="TRANSACTION")
								return($this->SetError("connection is not in TRANSACTION state"));
		...
		Function stream_open($path, $mode, $options, &$opened_path)
		{
			...
			$url=parse_url($path);
			$host = $url['host'];
			$pop3 = &Pop3_Core::SetConnection(0, $host, $this->pop3);
			if(IsSet($pop3))
			{
				$this->pop3 = &$pop3;
